...
import { VOLTAGE_PAGES, getIconForVoltage, getColorForVoltage } from '../../lib/constants';
...
export async function generateStaticParams() {
    return VOLTAGE_PAGES.map((page) => ({
        slug: page.slug.replace(/^\//, ''), // Remove leading slash if present in slugs
    }));
}
...
export default async function VoltageDetail({ params }: { params: Promise<{ slug: string }> }) {
    const { slug } = await params;
    const config = VOLTAGE_PAGES.find((p) => p.slug === `/${slug}`);
                ...
                <div className="space-y-8">
                    ...
                    <div className="bg-white border-2 border-slate-900 p-0 shadow-lg">
                        ...
                        <div className="p-6">
                            ...
                            {/* Dynamic Color Chart */}
                            <VoltageChart data={chartData} accentColor={accentColor} />
